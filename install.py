...
class dmb_zk(public):
      ...
      def config(self):
          ...
          if os.path.exists('/data/app/ZooKeeper'):
             print  'check   /data/app/ZooKeeper         \033[32m[OK]\033[0m.'
          else:
             ZooKeeper_zip='/tmp/ZooKeeper.zip' 
             self.unzip(ZooKeeper_zip ,'/data/app') 
             print  'install   ZooKeeper         \033[32m[OK]\033[0m.'
      ...
      def modify(self):
          IP=self.get_local_ip()[0]
          conf_dict=user_conf.info.get(IP)
          a=conf_dict.get('myid')
          b=conf_dict.get('server')
     
          zoo_file=file(user_conf.app_config_file,'r')
          zoo_info=zoo_file.read()
          zoo_file.close()
         # print  zoo_info
          if re.findall(IP,zoo_info) and re.findall(a,zoo_info):
             print  'check   zoo_cfg             \033[32m[OK]\033[0m.'
          else:
             zoo_file=file(user_conf.app_config_file,'a')
             zoo_file.write(b)
             zoo_file.close()
          
          data_file=file('/data/app/ZooKeeper/RegisterCenter/data/myid','w')
          data_file.write(a)
          data_file.close()
          
          dataLog_file=file('/data/app/ZooKeeper/RegisterCenter/dataLog/myid','w')
          dataLog_file.write(a)
          dataLog_file.close()
